 ...
 * file tree, and (c) classify each file via lookup tables in LLM context â€” a
 ...
 *
 * Output JSON (subset of what project-scanner.md Phase 1 expects â€” the LLM
 ...
 *   `Warning: scan-project: <path> â€” <reason> â€” file skipped from output`
...
// ---------------------------------------------------------------------------
const createIgnoreFilter = () => ({ isIgnored: (rel) => false });
 ...
 * Includes the legacy Step-3 mapping (.cfg/.ini/.env -> `config`) â€” note
...
const LANGUAGE_BY_EXT = Object.freeze({
  ...
  // lists them as code languages â€” downstream import map will return [])
  '.vue': 'vue',
  '.svelte': 'svelte',
  ...
  // treats them as config â€” see CATEGORY_BY_EXT)
  '.csproj': 'csproj',
  '.sln': 'sln',
 ...
 * then no-extension filename lookup. Never returns null â€” falls back to
...
export function detectLanguage(filePath) {
  ...
  // Dockerfile.dev, Dockerfile.prod, etc. â€” common variant form.
  if (base === 'Dockerfile' || base.startsWith('Dockerfile.')) return 'dockerfile';

  // Dotfile names like .env, .env.local â€” path.extname returns '' for
  ...
  if (ext) {
    ...
    // Unknown extension â†’ drop the leading dot, lowercase. Never null.
    return ext.slice(1);
  }

  // No-extension file â€” try filename table.
 ...
 * lookup is intentionally last-resort â€” many configs need their full path
 * inspected first.
 */
 ...
 * project-scanner.md Step 4 "Priority rule" â€” most specific wins.
 *
 ...
 * 5. Fallback: `code` (matches the spec â€” "All other extensions").
 */
export function detectCategory(filePath) {
  ...
  // Rule 2: infra by filename â€” Dockerfile + variants, Makefile,
  ...
  // `*.k8s.yml` and `*.k8s.yaml` â€” Kubernetes-flavored YAML.
  ...
  // path.extname misses these â€” see dotfileKey docstring.
  const dotKey = dotfileKey(base);
  if (dotKey) {
  ...
  // config). We don't enumerate every possible config filename here â€” that
...
function enumerateViaGit(projectRoot) {
  ...
  // bytes in path names â€” paths containing emoji, accented characters, CJK
  // codepoints, etc. come back quoted with octal escapes (e.g.
  // `"30. \360\237\217\227 BD-CCER/file.md"` for a path containing ðŸ—ï¸).
  ...
  const result = spawnSync('git', ['ls-files', '-z', '-co', '--exclude-standard'], {
    cwd: projectRoot,
    encoding: 'utf-8',
    maxBuffer: 256 * 1024 * 1024, // 256MB â€” huge monorepos can produce >10MB of paths
  });
...
/**
 * Recursive directory walker â€” fallback when `git ls-files` is unavailable
...
function enumerateViaWalk(projectRoot) {
  // Hard skip â€” these directories are universally non-source and skipping
  ...
  function walk(absDir) {
    ...
    try {
      ...
      process.stderr.write(
        ...
        `â€” directory read failed (${err.message}) â€” subtree skipped\n`,
      );
      return;
    }
    ...
    for (const ent of entries) {
      ...
      // Symlinks intentionally ignored â€” git ls-files doesn't follow them
 ...
 * order â€” caller is responsible for sorting + filtering.
 */
function enumerateFiles(projectRoot) {
  const fromGit = enumerateViaGit(projectRoot);
  if (fromGit !== null) return fromGit;
  process.stderr.write(
    `scan-project: git ls-files unavailable â€” falling back to recursive walk\n`,
  );
...
//
// Negation (`!pattern`) is correctly handled by the combined filter â€” a file
...
/**
 * Build a defaults-only IgnoreFilter â€” same patterns as createIgnoreFilter
 ...
 * one fewer than the visible line count â€” same behavior as wc.
 *
...
function countLines(absPath, posixPath) {
  try {
    ...
    process.stderr.write(
      `Warning: scan-project: ${posixPath} â€” line count failed ` +
      `(${err.message}) â€” file skipped from output\n`,
    );
    return null;
  }
}
...
async function main() {
  ...
  for (const rel of candidates) {
    ...
    // it, this is a baseline default drop â€” not counted. If defaults-only
  ...
  for (const rel of kept) {
    ...
    // Stat first â€” git ls-files could include paths that vanished between
    // listing and processing; the walker shouldn't but defensive anyway.
    try {
      const st = statSync(absPath);
      if (!st.isFile()) {
        // Symlinks-to-dir, special files, etc. â€” skip silently. Not a
        // warning condition because git wouldn't have tracked it as a file.
        continue;
      }
    } catch (err) {
      process.stderr.write(
        `Warning: scan-project: ${rel} â€” stat failed (${err.message}) ` +
        `â€” file skipped from output\n`,
      );
      continue;
    }
