...
/**
 * Helper chứa các logic UI dùng chung cho các menu controller. Trích xuất để
 * tránh duplicate giữa BaseMenuController và AssetsListController (DRY).
 */
public class MenuHelper {
    ...
    public static void handleTransaction() {
        if (SessionManager.getInstance().isBidder()) {
            showAmountDialog("Nạp tiền", "Nhập số tiền muốn nạp:").ifPresent(amount -> ClientNetworkManager
                    .getInstance().sendData(Protocol.REQ_DEPOSIT + Protocol.DELIMITER + amount));
        } else if (SessionManager.getInstance().isSeller()) {
            showAmountDialog("Rút tiền", "Nhập số tiền muốn rút:").ifPresent(amount -> ClientNetworkManager
                    .getInstance().sendData(Protocol.REQ_WITHDRAW + Protocol.DELIMITER + amount));
        }
    }
    ...
    public static void setupBalanceLabel(javafx.scene.control.Label balanceLabel) {
        ...
        SessionManager.getInstance().balanceProperty().addListener((obs, oldVal, newVal) -> javafx.application.Platform
                .runLater(() -> balanceLabel.setText(String.format("💰 Số dư: $%,.0f", newVal.doubleValue()))));
    }
}
